...
public class BleManager {
    ...
    public void write(byte[] data, BleDevice tempDevice, String toString, String toString1, BleWriteCallback bleWriteCallback) {
        if (bleWriteCallback == null) {
            throw new IllegalArgumentException("BleReadCallback can not be Null!");
        }

        BleBluetooth bleBluetooth = multipleBluetoothController.getBleBluetooth(tempDevice);
        if (bleBluetooth == null) {
            bleWriteCallback.onWriteFailure(new OtherException("This device is not connected!"));
        } else {
            bleBluetooth.newBleConnector()
                    .withUUIDString(toString, toString1)
                    .writeCharacteristic(data, bleWriteCallback, toString1);
        }
    }
